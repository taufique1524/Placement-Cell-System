...
const addOpening = asyncHandler(async (req, res) => {
    ...
    let formattedBranchesAllowed = [];
    if (Array.isArray(branchesAllowed)) {
        formattedBranchesAllowed = branchesAllowed.map(ele => ele.trim());
    } else if (typeof branchesAllowed === "string" && branchesAllowed.length !== 0) {
        formattedBranchesAllowed = branchesAllowed.split(",").map(ele => ele.trim());
    }
    ...
    if (offerType === "Internship") {
        ...
        if (flag === 0) {
           ...
           // console.log('Opening data to be saved:', data);
...
const updateOpening = asyncHandler(async (req, res) => {
    ...
    // Validate required fields (batch, companyName, etc.)
    let error = {};
    let flag = 0;
    ...
    if (!updateData.offerType || updateData.offerType === "") {
        error.offerTypeError = "Offer type can't be empty";
        flag++;
    }
    ...
    if (!updateData.branchesAllowed || updateData.branchesAllowed.length === 0) {
        error.branchesAllowedError = "Branches allowed can't be empty";
        flag++;
    }
    if (!updateData.batch || updateData.batch === "") {
        error.batchError = "Batch can't be empty";
        flag++;
    }
    if (flag > 0) {
    ...
    // Update the opening
    const updatedOpening = await Opening.findByIdAndUpdate(openingId, updateData, { new: true });
    if (!updatedOpening) {
        return res.status(404).json({ success: 0, message: "Opening not found" });
    }
    res.json({ success: 1, opening: updatedOpening });
});

module.exports = { addOpening, getAllOpenings, deleteOpening, getSingleOpening, updateOpening };