...
function Registration() {
  ...
  // If coming from email verification, auto-fill and lock email
  useEffect(() => {
  ...
  // Always keep formData.email in sync with verifiedEmail if present
  useEffect(() => {
    if (location.state?.verifiedEmail && formData.email !== location.state.verifiedEmail) {
      setFormData(prev => ({
        ...prev,
        email: location.state.verifiedEmail
      }));
    }
  }, [location.state?.verifiedEmail, formData.email]);

  async function handleSubmit(e) {
    e.preventDefault();
    try {
      setLoading(1);
      // Always use verifiedEmail if present
      const submitData = {
        ...formData,
        email: location.state?.verifiedEmail || formData.email
      };
  ...
  const handleChange = (e) => {
    // Prevent editing email if verifiedEmail is present
    if (e.target.name === "email" && location.state?.verifiedEmail) {
      return;
    }
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <Input
                  ...
                  name="email"
                  id="email"
                  value={location.state?.verifiedEmail || formData.email}
                  onChange={handleChange}
                  className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                  placeholder="name@company.com"
                  required=""
                  disabled={!!location.state?.verifiedEmail}
                />
